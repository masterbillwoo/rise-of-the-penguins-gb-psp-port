...
//Set whenever a menu or prompt closes. The button that dismissed it is still
//physically held, and ControlsUpdate would otherwise hand it straight to the
//game - closing the menu with Circle pressed B in-game. Cleared once the pad
//is released.
//Frames left to hold input back for; see ControlsUpdate
int gblSwallowInput = 0;

void ControlsShowMenu()		{
	SoundPause();
	gblNewGame = 0;
	MenuOptionsConfigure(-1);
	//Our own vertical menu, not MasterBoy's carousel (see psp/gamemenu.c).
	//menuPlusShowMenu() is still used at boot, where it loads config and fonts.
	GameMenuShow();
	if (gblNewGame)
...
//	MyReadKeys();
	...
	//Hold the game's input back until whatever closed the menu is let go. Counted
	//down as well as released: a drifting analog stick keeps setting direction bits
	//through menuKeysAnalogApply, which would otherwise freeze input for good.
	if (gblSwallowInput > 0)		{
		gblSwallowInput--;
		if (pad1->Buttons)
			pad1->Buttons = 0;
		else
