...
/*
	...
	(c'est l� juste pour fixer les pertes de performances dans certaines build)
	R�f�rence: 82% sonic 1 jungle zone
*/
...
OSL_CONTROLLER *MyReadKeys()
{
	...
	*/
	...
	//Trop long => d�sactive le stick pour le menu (il y a un probl�me)
				...
				//Autorepeat pour cette touche?
					...
					if (menuPressedKeys.acuts[key] >= menuPressedKeysAutorepeatInit)			{
						...
						if ((menuPressedKeys.acuts[key] - menuPressedKeysAutorepeatInit) % menuPressedKeysAutorepeatDelay == 0)			{
							//D�clenchement une nouvelle fois
...
/*	if (keyFound >= 0)			{
		//Ne pas �tre prise en compte par les autres
...
int menuGetMenuKey(u32 *pad, int key)		{
	if (menuPressedKeys.acuts[key])
		//Pour simuler comment �a se passait avant
...
int fctMainVideoOverlay(SUBMENU *menu, SUBMENUITEM *sub, u32 event)
{
	return GestionEventStandardInt(menu, sub, event, MENUUPD_VIDEO, &menuConfig.video.overlay);
}
...
/*	if (slot >= 0 && slot <= 9)		{
	...
	if (bAddExtension)		{
		if (slot == STATE_EXPORT && gblMachineType == EM_SMS)
			...
			else if (slot == STATE_AUTO)
				format = "%s/SAVE/%s.auto.gz";
			else
...
void OuvreFichierPalette(int crc, char *fonction)		{
	if (gblModeColorIt)			{
		if (fonction)
			...
			tilePalReinit();
			//Si la section correspondant au crc actuel n'existe pas, on utilise le profil par d�faut [default].
...
char *menuGetKeyName(char *dest, u32 maxLen, char *separator, u32 key)		{
	...
	int keyorder[28]=		{
		...
		8, 9,										//L/R
		3, 0,										//Start/Select
		12, 13, 14, 15,								//Triangle/Rond/Croix/Carr�
...
int menuRedefineGetNewKey(SUBMENUITEM *sub)		{
	...
	do			{
		//S'il appuie une touche et la l�che, �a la prend en compte, mais pas les fl�ches
...
/*		//R�cup�re la touche
		...
		//V�rifie que ce n'est pas la m�me qu'une autre!
		for (i=0;i<NBRE_TOUCHES;i++)	{
...
int fctMainCtrlShortcuts(SUBMENU *menu, SUBMENUITEM *sub, u32 event)
{
	...
	if (event == EVENT_SELECT)		{
		...
		//Le seul cas o� il est permis d'avoir deux fois la m�me touche: (none) ;)
		if (i >= NBRE_TOUCHES || !result)
			menuConfig.ctrl.acuts[sub->prop1] = result;
		//La touche existe d�j� (faisons attention � ce que ce ne soit pas le m�me raccourci)
		else if (i != sub->prop1)		{
			...
			//On �change les touches dans ce cas
...
//Overlay filters. Static on purpose: building this array at run time reliably
//crashed the menu system, so the list is fixed and OverlayLoad() maps each index
//onto a file in overlays/. Swap a PNG's contents to restyle an entry.
SUBMENUITEM menuMainVideoOverlayItems[]=		{
	{"Off", "No overlay", "", NULL, 0},
	{"LCD fine", "Soft 2px LCD texture", "", NULL, 1},
	{"LCD coarse", "Chunkier 3px dots", "", NULL, 2},
	{"Scanlines", "Horizontal lines only", "", NULL, 3},
	{"Recessed", "Screen shadow, no grid", "", NULL, 4},
	{"Pixel grid", "Locked to GB pixels; can band", "", NULL, 5},
};

SUBMENU menuMainVideoOverlay=		{
	NULL, numberof(menuMainVideoOverlayItems),
	2,			//Choice
	menuMainVideoOverlayItems,
	fctMainVideoOverlay
};
...
//Mettre un lien vers un menu principal est interdit!!! Mettre un lien vers le menu parent le fermera (autre parent non support�).
...
SUBMENUITEM menuMainVideoRenderItems[]=		{
	...
	{"Overlay", "Draw overlay.png over the game (LCD grid / bezel art)", "", &menuMainVideoOverlay},
};
...
void fctMsgBoxAbout_Handle(WINDOW *w, int event)		{
	...
	if (event == EVENT_DRAW)		{
		...
		//Position de la fen�tre (pour clipping)
...
/*		HandleSubMenu(&menuMsgBoxSaveConfig);
		//Ne jamais laisser g�rer �a par la bo�te sinon elle se fermera!
...
//		oslDrawString(x + MENU_LEFT_MARGIN + 1, y, s->titre);
	...
	if (s->type == 3)			{
		...
		yText = y;
		if (s->state.active > 0)		{
			//Petit effet sp�cial pour la s�lection
...
void HandleSubMenu(SUBMENU *s)		{
	...
	if (s->active)			{
		...
		if (s->type == 3)			{
			...
			//Rien n'a chang�? => pas d'emphase sur la fl�che
			i = GetIntFromDigits(mm);
			...
			//D�placement horizontal quand un choix est s�lectionn�
	...
	/*						ss = s;
					...
					//Ne plus g�rer, sauf pour les choix
...
void menuDrawIcons(MENU *m, int angle, int ajoutRayon)
{
	...
	for (i=0;i<m->nbItems;i++)		{
		...
		stretchX = 32.f * scale;
		stretchY = 32.f * scale;
		//Bilin�aire si redimensionnement
...
void InitConfig()
{
	...
	menuConfig.file.autoState = 1;
	//Resume straight away by default; the prompt is opt-in
	menuConfig.file.autoStateLoad = 2;
	//Do not drop the player straight into play with no warning
	menuConfig.file.startPrompt = 1;
	...
	menuConfig.sound.masterVolume = 100;
...
/*	//Syst�me
	...
	if (sceUtilityGetSystemParamInt(PSP_SYSTEMPARAM_ID_INT_LANGUAGE, &iValue)!=PSP_SYSTEMPARAM_RETVAL_FAIL)
		menuLanguage = iValue;					//D�faut: PSP_SYSTEMPARAM_LANGUAGE_ENGLISH
	...
	//InitConfig() filled menuConfig with the built-in defaults above but never
	//stored them, leaving menuConfigDefault as uninitialised heap - so every
	//"revert to default" read garbage. Capture them now.
	memcpy(menuConfigDefault, &menuConfig, sizeof(MENUPARAMS));
}
...
//		menuMainVideoRenderItems[4].disabled = (menuConfig.video.brightnessMode == 1);
		...
		//Frameskip max toujours sup�rieur au frameskip
		...
		RangeSetSubMenuValueInt(&menuMainVideoRenderItems[8], &menuConfig.video.overlay, 1);
...
void DrawWindow(WINDOW *w)		{
	...
	//S'assure que les fichiers vitaux sont charg�s
	if (!imgBord)
		menuLoadFilesMin();
...
void HandleWindow(WINDOW *w)		{
	...
	if (w->state == 1 && w->buttons != MB_NONE)		{
		...
		if (osl_keys->pressed.circle)
			CloseWindow(w, 0);
		//Les autres ne doivent pas g�rer de touche dans ce mode!
		osl_keys->pressed.value = 0;
	}
	//Fermer la fen�tre
	...
	/*		oslDrawGradientRect(0,0,480,272,RGB(0,0,0), RGB(0,255,0), RGB(255,0,0), RGB(0,0,255));
		...
		if (pass == 0)			{
			if (osl_keys->analogY == -128)
				...
				if (abs(osl_keys->analogY - curValue) >= 5)				//Trop boug�
		...
		//Left
			...
			if (osl_keys->analogX == -128)
				...
				if (abs(osl_keys->analogX - curValue) >= 5)				//Trop boug�
...
void InitMenus()		{
	...
	//D�sactive les options inutiles (reset SMS et Screenshot)
...
void LoadUserDefaultConfig()		{
	//Recopie la config. par d�faut
	OuvreFichierConfig("default.ini", NULL);
	//sizeof(&ptr) is 4 bytes; these used to copy a pointer's worth and leave the
	//rest stale, so the shipped DEFAULT.INI values were never recorded.
	memcpy(menuConfigUserDefault, &menuConfig, sizeof(MENUPARAMS));
	memcpy(menuConfigUserMachineDefault, &menuConfig, sizeof(MENUPARAMS));
}
...
{
	if (event == EVENT_SET)		{
		//Premi�re possibilit� dispo uniquement en jeu
		if (!menuIsInGame)
...
{
	//Batterie
	ScePspDateTime local_time;	//was pspTime in the old PSPSDK
	batteryPercentage = -1;
	//L'heure
	...
	if (scePowerIsBatteryExist()) {
		...
		sprintf(menuInfoBattery, "%02i:%02i", local_time.hour, local_time.minute);
}

//=== SINGLE-GAME LAUNCHER MODE ===
//Looks for a ROM inside a "roms" folder sitting next to the EBOOT. If exactly one
//is wanted we simply take the first match, so the file keeps its original name
//(useful when shipping several language variants of the same game).
//Returns 1 and fills dest with a full path on success, 0 if there is nothing to load.
int SingleGameFindRom(char *dest, int destSize)
{
	static const char *exts[] = {".gb", ".gbc", ".sms", ".gg", NULL};
	char dir[MAX_PATH];
	SceUID d;
	SceIoDirent ent;
	int found = 0;
	...
	if (gblAppPath[0])
		snprintf(dir, sizeof(dir), "%s/roms", gblAppPath);
	else
		strcpy(dir, "roms");
	...
	while (!found && sceIoDread(d, &ent) > 0)		{
		...
		int i;
		//Skip "." / ".." and dot-files such as the "._name" forks a Mac leaves behind
		if (ent.d_name[0] != '.' && !(ent.d_stat.st_attr & PSP_FILE_TYPE_DIR) && e)		{
			for (i = 0; exts[i]; i++)		{
				if (!strcasecmp(e, exts[i]))		{
					snprintf(dest, destSize, "%s/%s", dir, ent.d_name);
					found = 1;
					break;
				}
			}
...
void menuPlusShowMenu()
{
	...
	static int bSingleGameFirstBoot = 1;
...
//	menuMainSelectedOption = 0;
	...
	//=== SINGLE-GAME LAUNCHER MODE ===
	//Everything above still runs (user config, skins, fonts, menu state), so the
	//in-game menu keeps working; we only skip *showing* the browser. MenuPlusAction
	//sets menuPlusTerminated, so the loop below never iterates and we fall straight
	//through to the normal clean-up at the end of this function. If it returns 0 the
	//ROM failed to load and the menu is shown as usual.
	if (bSingleGameFirstBoot)		{
		...
		if (!menuIsInGame && SingleGameFindRom(singleRom, sizeof(singleRom)))		{
			if (MenuPlusAction(MA_LOADROM, singleRom))		{
				...
				//fadeReason == 1 branch): machine defaults, then the per-game
				//config, then the battery save - SRAM_LOAD is only issued there,
				//so without this the player's save would never be restored.
				//The browser gates this on gblFlagLoadParams, which it sets itself
				//and which is cleared by its "load without parameters" shortcut;
				//we never enter it, so we always load with parameters.
				LoadDefaultMachineConfig();
				pspGetStateNameEx(singleRom, temp, STATE_CONFIG);
				OuvreFichierConfig(temp, NULL);
				if (gblMachineType == EM_GBC)		{
					gb_reset();
					machine_manage_sram(SRAM_LOAD, 0);
				}
				menuMainFileItems[2].disabled = (gblMachineType != EM_GBC);

				//Is there a state to resume from? The load itself waits until the
				//emulator is initialised - restoring one into a half-built machine
				//is asking for trouble - so only the flag is set here.
				if (menuConfig.file.autoStateLoad)		{
					char statePath[MAX_PATH];
					pspGetStateNameEx(singleRom, statePath, STATE_AUTO);
					if (fileExists(statePath))
						gblPendingAutoLoad = 1;
				}
			}
		}
	}
	...
	if (menuMusicState != 3 && !menuPlusTerminated)
		menuMusicPlay();
	menuSetStatusBarMessageDirect(NULL);
	...
	while (!osl_quit && !menuPlusTerminated)
	{
		...
		if (!gblChoice.active)			{
			...
			if (osl_keys->pressed.left || osl_keys->pressed.right)			{
				...
				//Actif (dessin�) mais ne g�re pas les touches (nouvelle option)
				s->active = 1;
				s->moving = moving;
				s->movingDist = 0;
				s->movingAlpha = 255;
		...
		//Premi�re animation (entr�e du menu en cercle)
		if (!menuEndingFade)
			nRayon -= 7;
		if (nRayon > 0)			{
			...
			float tmpMenuAngle;
			//Arriv�e du cercle termin�e
			nRayon = 0;
			tmpMenuAngle = adapteAnglef(menuDesiredAngle, menuAngle);
			//Animation de tournant termin�e
...
//		HandleSubMenu(menuMainItems[menuMainSelectedOption].subMenu);
		...
		if (menuMainFileStateLoad.active == 1 || menuMainFileStateSave.active == 1 || menuMainFileStateDelete.active == 1)		{
			...
			//<0 = d�sactiv�, r�active si l'option a chang�
		...
		//Ne pas afficher le hint pendant ce temps-l�
		if (nRayon > 0)
			gblHint.active = 0;
		HandleHint(&gblHint);
...
//		}
	...
	if (menuGetMenuKey(&buttons, MENUKEY_SCREENSHOT))		{
		...
		//Ne pas garder le state en m�moire quand on est ingame
		if (menuTempStateMemory)		{
			free(menuTempStateMemory);
			menuTempStateMemory = NULL;
		}
...
void menuSoundShowDisplay()			{
	...
	if (menuSoundBoxActive > 0 || menuSoundBoxOpen)		{
		//La premi�re fois, calcule la taille max
