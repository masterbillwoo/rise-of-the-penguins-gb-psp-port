...
//Absolute path of the directory holding the EBOOT, e.g. "ms0:/PSP/GAME/MyGame".
//Empty if it could not be determined, in which case callers fall back to relative
//paths (which depend on the thread cwd the kernel gives us and are less reliable).
char gblAppPath[MAX_PATH] = "";

static void InitAppPath(int argc, char *argv[])
{
	char *p;
	...
	strncpy(gblAppPath, argv[0], sizeof(gblAppPath) - 1);
	gblAppPath[sizeof(gblAppPath) - 1] = '\0';

	//Strip the "/EBOOT.PBP" component, leaving the containing directory
	p = strrchr(gblAppPath, '/');
	if (!p)
		p = strrchr(gblAppPath, '\\');
	if (p)
		*p = '\0';
...
{
#ifdef KERNEL_MODE
	strcpy(gblAppPath, startPath);
#else
	InitAppPath(argc, argv);
#endif
