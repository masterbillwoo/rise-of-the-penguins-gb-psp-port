...
//Valeur de l'option par d�faut des menus
...
typedef struct		{
	struct			{
		...
		int state;
		int sramAutosave;			// battery/cartridge save written automatically
		int autoState;				// 0 = off, 1 = write a save state when quitting
		int autoStateLoad;			// 0 = never, 1 = ask, 2 = always resume on launch
		int startPrompt;			// wait for a button before play begins
		int wifiEnabled;
		int wifiServer;
	} file;
	...
	struct			{
		...
		int overlay;				// grid style: 0 = off, else index into overlayFiles
		int bezel;					// shell art: 0 = off, else index into bezelFiles				// 0 = off, 1 = on; draws overlay.png over the game
	} video;
	...
	struct			{
		...
		int volume;					// 1 = plus fort, 0 = normal (music player boost)
		int masterVolume;			// 0-100 %, applied to the emulator's audio output
...
#define STATE_SRAM -1
#define STATE_RAM 10
//Auto save state: its own slot so it never collides with the player's 0-9
...
//Directory containing the EBOOT (set in main.c), empty if unknown
extern char gblAppPath[MAX_PATH];
extern int SingleGameFindRom(char *dest, int destSize);
//Set when a resume state is waiting; consumed once the emulator is running
extern int gblPendingAutoLoad;
//Set on menu close so the dismissing press is not delivered to the game
extern int gblSwallowInput;
extern int pspSaveState(int slot);
extern int pspLoadState(int slot);
extern void pspGetStateNameEx(char *romFile, char *name, int slot);
extern void menuPlusShowMenu();
extern void UpdateMenus(int elements);
//Retourne si une touche est press�e ou non
...
typedef struct SUBMENUITEM 		{
	...
	char value[20];
	struct SUBMENU *link;
	int prop1, prop2;					//Propri�t�s (perso)
	char disabled;						// 0 = actif, 1 = inactif, +2 = couleur sp�ciale (prop2)
} SUBMENUITEM;
...
typedef struct WINDOW		{
	...
	const char *button1;
	const char *button2;
	//Fonction qui sera appel�e � la fermeture (mettre NULL pour bloquer et retourner un r�sultat)
