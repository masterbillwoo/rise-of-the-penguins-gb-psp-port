//Brunni's portable config system (qui d�chire trop sa race... hem en fait non)
#include "pspcommon.h"
...
//Indique que ce n'est pas sauv� automatiquement
...
//Octets utilis�s pour les types
#define MASQUE_TYPE 15
//Taille max (0 � 15: 0 = 1, 1 = 2, 2 = 3, 4 = 16, etc.)
...
char *FctGoto(int type, char *ptr)		{
	//Note: les labels ne peuvent �tre qu'en dessous
...
char *FctPalette(int type, char *ptr)			{
	...
	//Pour ne pas avoir besoin de faire un end � la fin
	arretdurgence = 1;
	return NULL;
}
...
char *FctTabMenuColCursor(int type, char *ptr)		{
	//Erreur => on doit le faire nous m�me...
	if (type == FCT_SET)		{
		int valeur;
...
void FctColorIt_AddTileRule(int type, char *ptr)		{
	...
	//V�rifie que les valeurs fournies sont correctes
	if (tileStart < 0 || tileStart >= 384 || tileEnd < 0 || tileEnd >= 384 || palNo < 0 || palNo >= NB_PALETTES)
		return;

	//D�finit les palettes des tiles
...
void FctColorIt_AddTileCrc(int type, char *ptr)		{
	...
	if (tileStart < 0 || tileStart >= 384 || tileEnd < 0 || tileEnd >= 384)
		return;
	//Savoir si une tile a son crc compt�:
...
void FctColorIt_SetTilesetData(int type, char *ptr)		{
	...
	//Mode normal: donn�es RAW au format GB
	if (mode == 'N')		{
		//16 bytes per tile
		for (i=0;i<16;i++)		{
			//R�cup�ration d'un nombre (doublet) hexad�cimal
			u8 value1, value2;
			do	{
				value1 = *text++;
			} while (value1 == ' ');
		...
		for (j=0;j<8;j++)		{
			for (i=7;i>=0;i--)		{
				//R�cup�ration d'un nombre
				u8 value;
				do	{
					value = *text++;
				} while (value == ' ');

				//Affectation des bits selon la valeur (1 � 4)
				if (value == '0' || value == '3')
					tilePtr[0] |= 1 << i;
				else
					tilePtr[0] &= ~(1 << i);
...
void FctColorIt_SetTile(int type, char *ptr)		{
	...
	while (redo--)		{
		...
		//Un troisi�me argument?
		if (*ptr)
			//Ne pas prendre si 512 a �t� ajout� car �a n'est valable que pour newTile
			tileEnd = newTile & 511;
		else
			redo = 0;
	}
...
const IDENTIFICATEUR objFile[]=	{
	...
	{"autoState",			&menuConfig.file.autoState,			TYPE_BOOL},
	{"autoStateLoad",		&menuConfig.file.autoStateLoad,		TYPE_ENTIER},
	{"startPrompt",			&menuConfig.file.startPrompt,		TYPE_BOOL},
};
...
const IDENTIFICATEUR objVideo[]=	{
	...
	{"overlay",				&menuConfig.video.overlay,			TYPE_ENTIER},
	{"bezel",				&menuConfig.video.bezel,				TYPE_ENTIER},
...
const IDENTIFICATEUR objSound[]=	{
	...
	{"masterVolume",		&menuConfig.sound.masterVolume,		TYPE_ENTIER},
...
const IDENTIFICATEUR objGameBoy[]=	{
	...
	//TYPE_MAXSIZE was missing: maxSize is 1 << mask, so with no mask the name was
	//truncated to a single byte and a palette could never be loaded from a config
	//file. The field is char[32], hence 1 << 5.
	{"palette",				&menuConfig.gameboy.palette,		TYPE_CHAINE | TYPE_MAXSIZE(5)},
...
//Comparaison entre une cha�ne � casse basse et mixte
		...
		if (type & SAVETYPE_CHANGESONLY)		{
			...
			if (o->objet[i].type == TYPE_TABLEAU)			{
				...
				if (adr)		{
					...
					//Aucune entr�e diff�rente trouv�e
...
void ScriptAjouteProcedures(char *nom_fichier, SUBMENU *s, char *startText)		{
	...
	while(!VirtualFileEof(f))		{
		...
		//Tout d'abord: ne pas �tre "case sensitive"...
		for (i=0;str[i];i++)	{
//			if (str[i]>='A' && str[i]<='Z')
//				str[i]+=32;
...
parsingTermine:
		...
		//Sauter les espaces/tabs �ventuels
		i=j=0;
		while(str[i]==' ' || str[i]=='	')
			i++;

		//Bon, c'est pas le tout, mais on a un label � trouver nous!
		j=i;
		...
		if (!str[i])									//Pas un label
			continue;
		str[i]=0;
		if (str[j]!='_')								//Pas un label export� (_label:)
			continue;
		ScriptAjouteProcedures_Add(s, str + j + 1);
	}

	//Met � jour les pointeurs (qui bougent avec realloc)
...
const IDENTIFICATEUR *TrouveIdentificateur(const IDENTIFICATEUR *source, int taille, char *objet)
{
	...
	//Trouve la r�f�rence d'objet dans la table
	for (i=0;i<taille;i++)		{
...
void SetVariableValue(const IDENTIFICATEUR *id, char *ptr)		{
	...
	switch (id->type & MASQUE_TYPE)			{
		case TYPE_FONCTION:
			//Depuis que je m'amuse sur GBA avec ce genre de trucs, �a me semble d�j� plus s�r comme m�thode. Ne vous inqui�tez pas si vous trouvez �a sale :p
...
//WithQuotes: pour inclure les guillemets autour des cha�nes et autres
		...
		case TYPE_TABLEAU:
			...
			for (i=0;i<it->taille;i++)			{
				...
				if (val)		{
					...
					//S�pare les �l�ments d'une virgule, sauf le dernier
					if (i < it->taille - 1)
...
parsingTermine:
		if (str[i]==':' && !trouveLabel)		//Label trouv�!
			continue;
...
continueParse:
		...
		if (trouveLabel)		{
			...
			if (strlwrmixcmp(str+j, trouveLabel))			//Pas le bon label
				continue;
			trouveLabel=NULL;								//Label trouv� -> plus � rechercher.
			continue;
		}

		//Apr�s, r�cup�rer l'objet de la commande (op�rande gauche)
		while(str[i] && str[i]!=' ' && str[i]!='	' && str[i]!='.' && str[i]!='=' && str[i]!='[')
			objet[j++]=str[i++];

		//Si �a n'a pas �t� termin� par un point
		...
		//Permet ensuite de savoir si aucune commande n'a �t� donn�e
		commande[0]=0;

		if (str[i]=='=' || str[i]==' ' || str[i]=='.' || str[i]=='[')				//Pas de m�thode
			goto sauteEspaces;

		//Fin de cha�ne -> seul un objet est sp�cifi�
		if (!str[i])
			goto sauteEspaces;
...
//commande:
		//Apr�s, r�cup�rer la commande � ex�cuter (op�rande droite)
		j=0;
...
//			goto commande;
		...
		//On est pr�t pour ex�cuter une commande!
		ptr=str+i;
...
//						return ((char* (*)(int, char*))idObjet->handle2)(FCT_SET_BEFORE, ptr);
					...
					//Le tableau ne peut �tre �valu� (erreur de syntaxe)
					if (idObjet->flags & FLAG_CALLBACK)
...
error1:
				...
				switch (it->type & MASQUE_TYPE)			{
					...
					default:
						//Vous devriez pouvoir adapter pour d'autres types sans trop de probl�mes ;)
...
continueParse:
	...
	if (str[i]==' ' || str[i]=='.' || str[i]=='[')				//Pas de m�thode
		goto sauteEspaces;
