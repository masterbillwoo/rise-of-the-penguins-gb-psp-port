...
char gblNewGame = 0;
//A resume state is waiting to be loaded (set during boot in menuplus.c)
int gblPendingAutoLoad = 0;
uLong sram_crc;
...
//Set while the timed flush is writing, so it never raises the low-battery prompt
//mid-game. A modal appearing unbidden during play would be worse than the risk it
//warns about, and the flush only writes when the CRC says the save really changed.
int gblSramSilentSave = 0;
...
//Called once per emulated frame. Stock MasterBoy wrote the battery save only on
//power-off, on loading another ROM, or from the menu's manual command - and its
//power-switch callback is commented out - so a crash, a flat battery or a yanked
//Memory Stick lost everything since boot. This flushes on a timer instead.
//machine_manage_sram compares a CRC first, so a tick with nothing changed costs one
//pass over the SRAM and no I/O at all.
void machine_sram_autosave_tick(void)
{
	static int counter = 0;

	if (!menuConfig.file.sramAutosave || menuConfig.file.filename[0] == '\0')
		return;
	if (gblMachineType == EM_GBC && !(rom_get_loaded() && rom_has_battery()))
		return;

	if (++counter < SRAM_AUTOSAVE_FRAMES)
		return;
	counter = 0;
...
static void SmsTerm(void)
{
	...
	{
		//=== AUTO SAVE STATE ===
		//Written before power-off, so quitting - from the menu or from HOME - can
		//be picked up exactly where it left off next launch.
		if (menuConfig.file.autoState && menuConfig.file.filename[0])
			pspSaveState(STATE_AUTO);
...
//	MenuPlusAction(MA_LOADROM, "ms0:/PSP/GAME/SMSPlus1.2/roms/Sonic Blast.sms");
	...
	//=== BOOT PALETTE ===
	//MenuOptionsConfigure(0) applies the palette earlier in this function, but the
	//machine is reset after that, so a palette coming from DEFAULT.INI never
	//survived to the first frame - picking one from the menu worked, loading one
	//from the config did not. Re-apply now that the machine is up.
	...
	{
		int resumed = 0;
		if (gblPendingAutoLoad)		{
			char statePath[MAX_PATH], marker[MAX_PATH];
			SceUID mfd;

			gblPendingAutoLoad = 0;
			pspGetStateNameEx(menuConfig.file.filename, statePath, STATE_AUTO);
			snprintf(marker, sizeof(marker), "%s.try", statePath);

			mfd = sceIoOpen(marker, PSP_O_RDONLY, 0777);
			if (mfd >= 0)		{
				//Last launch died part way through restoring this snapshot. Drop it
				//rather than crashing on every boot from now on.
				sceIoClose(mfd);
				sceIoRemove(marker);
				sceIoRemove(statePath);
			}
			else if (menuConfig.file.autoStateLoad == 2 || GameMenuAskResume())		{
				mfd = sceIoOpen(marker, PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777);
				if (mfd >= 0)
					sceIoClose(mfd);
				resumed = pspLoadState(STATE_AUTO);
				//Got here in one piece, so the snapshot is good
				sceIoRemove(marker);
			}
		}
...
/*		int autoFskip = (menuConfig.video.vsync == 2);
		...
		//Flush the battery save periodically rather than only on exit
		machine_sram_autosave_tick();
