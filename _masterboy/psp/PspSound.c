...
static int SoundThread()
{
	...
	if(chan > 0 && sample_count > 0 && sample_count <= 2048)
	{
		for(; soundRunning; )
		{
			...
			{
				//Master volume. sound.volume above is only the music player's boost;
				//this scales what the emulator itself outputs.
				int vol = menuConfig.sound.masterVolume;
				if (vol < 0)
					vol = 0;
				else if (vol > 100)
					vol = 100;
				sceAudioOutputBlocking(chan, (PSP_AUDIO_VOLUME_MAX * vol) / 100,
				                       (void*)audioOut[buf]);
			}
