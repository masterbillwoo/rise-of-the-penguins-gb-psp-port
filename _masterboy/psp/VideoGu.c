...
//=== SCREEN OVERLAY ===
//A single 480x272 RGBA image drawn over the finished frame. One mechanism covers
//both uses: an LCD pixel grid (semi-transparent lines over the play area) and
//bezel art (opaque pixels in the pillarbox margins). Generated by
//build/make_overlay.py, which aligns the grid to the scaling mode in use.
OSL_IMAGE *gblOverlay = NULL;
int gblOverlayLoaded = -1;
OSL_IMAGE *gblBezel = NULL;
int gblBezelLoaded = -1;
//char bVframeUncacheNeeded = 1;

#define ALTERNATE_FILLING
//D�fini dans VIDEOGU.H (utilis� dans menuplus.c aussi!)
//#define SKIP_BORDERS
...
/*	sceGuInit();
	...
	if (!videoInited)		{
		...
		oslStartDrawing();
		oslEndDrawing();
		//OSLib's built-in splash screens (2 = Neoflash logo, 1 = OSLib logo) are
		//boilerplate branding from the library and the emulator, not this game.
		//A single-game launcher should go straight to the title screen.
...
void VideoGuUpdate_Core(int fullScreen, int mustClear)		{
	...
	//Nothing has been emulated yet, so there is no picture to scale. Every branch
	//below divides by or scales against these, and a zero viewport gives garbage
	//destination coordinates rather than a trap on MIPS.
	if (bitmap.viewport.w <= 0 || bitmap.viewport.h <= 0)		{
		if (mustClear)
...
//Overlay filters, indexed by menuConfig.video.overlay. Index 0 is "Off"; the rest
//line up with menuMainVideoOverlayItems in menuplus.c and with the files shipped in
//overlays/. A fixed table rather than a directory scan: the menu list has to be
//static anyway, so there is nothing for a scan to feed.
static const char *overlayFiles[] = {
	NULL,
	"lcd_fine.png",
	"lcd_coarse.png",
	"scanlines.png",
...
#define OVERLAY_COUNT ((int)(sizeof(overlayFiles) / sizeof(overlayFiles[0])))

//Shell art is a separate layer: a bezel frames the screen, a grid textures the
//picture, and wanting both at once is the normal case. They were one list to begin
//with, which made them mutually exclusive for no good reason.
static const char *bezelFiles[] = {
	NULL,
	"bezel_gbc.png",
	"bezel_gb.png",
	"bezel_gbp.png",
};
#define BEZEL_COUNT ((int)(sizeof(bezelFiles) / sizeof(bezelFiles[0])))

//Loads the selected overlay. A missing or unreadable file just means no overlay -
//they are optional, so failure is not an error.
void OverlayLoad(void)
{
	char path[MAX_PATH];
	int sel = menuConfig.video.overlay;

	if (gblOverlay)		{
		oslDeleteImage(gblOverlay);
		gblOverlay = NULL;
	}
	gblOverlayLoaded = sel;

	if (sel <= 0 || sel >= OVERLAY_COUNT)
		return;

	if (gblAppPath[0])
		snprintf(path, sizeof(path), "%s/overlays/%s", gblAppPath, overlayFiles[sel]);
	else
		snprintf(path, sizeof(path), "overlays/%s", overlayFiles[sel]);

	//RAM rather than VRAM: at 480x272x4 this is 522KB and VRAM is needed for the
	//frame buffers and the scaled game texture.
	gblOverlay = oslLoadImageFilePNG(path, OSL_IN_RAM, OSL_PF_8888);
}

//Reloads if the menu changed the selection. Kept out of OverlayDraw so the file I/O
//and image allocation never happen between oslStartDrawing and oslEndDrawing.
//Same as OverlayLoad, for the shell art layer.
void BezelLoad(void)
{
	char path[MAX_PATH];
	int sel = menuConfig.video.bezel;

	if (gblBezel)		{
		oslDeleteImage(gblBezel);
		gblBezel = NULL;
	}
	gblBezelLoaded = sel;

	if (sel <= 0 || sel >= BEZEL_COUNT)
		return;

	if (gblAppPath[0])
		snprintf(path, sizeof(path), "%s/bezels/%s", gblAppPath, bezelFiles[sel]);
	else
		snprintf(path, sizeof(path), "bezels/%s", bezelFiles[sel]);

	gblBezel = oslLoadImageFilePNG(path, OSL_IN_RAM, OSL_PF_8888);
}

void OverlaySync(void)
{
	if (menuConfig.video.overlay != gblOverlayLoaded)
		OverlayLoad();
	if (menuConfig.video.bezel != gblBezelLoaded)
		BezelLoad();
}

void OverlayDraw(void)
{
	if (!gblOverlay && !gblBezel)
		return;

	oslSetAlpha(OSL_FX_ALPHA, 0xff);
	//Grid first, then the shell over it: the bezel is opaque outside the screen and
	//its inner shading should sit on top of the texture, not under it.
	if (gblOverlay)
		oslDrawImageXY(gblOverlay, 0, 0);
	if (gblBezel)
		oslDrawImageXY(gblBezel, 0, 0);
...
void VideoGuUpdate(int nFrame, int fullScreen)
{
	...
	//Before oslStartDrawing: loading an image mid-render list is not safe
	OverlaySync();
	...
	//Over the game, under the status/benchmark text
	OverlayDraw();
...
//		sceKernelDcacheWritebackInvalidateRange(pixels, 256 * 256 * 2);
	...
	//On dessine d'abord le haut de l'�cran: il faut le faire rapidement sinon un lcd_render ou autre viendra remplacer le buffer pendant qu'on dessine la derni�re bande!
